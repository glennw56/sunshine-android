...
3. **TIP VIA AD** — Android **AdMob rewarded** (Poing plugin, Godot 4.3). Credits a **FREE TIP to the STAFF jar** (not a customer perk). Debug builds use Google’s official test app/unit ids until CoS pastes production `ca-app-pub-…` ids into `[sunshine] admob_app_id` / `admob_rewarded_unit` (or `SUNSHINE_ADMOB_*`) and sets `ad_mode=live`. Editor/desktop still uses a mock overlay.
...
| `SUNSHINE_AD_MODE` | `test` | `mock` (in-engine overlay), `test` (AdMob Google test unit on Android), `live` (production unit) |
| `SUNSHINE_ADMOB_APP_ID` | Google sample `ca-app-pub-3940256099942544~3347511713` | AdMob application id (manifest). Replace with production `ca-app-pub-XXXX~YYYY` from AdMob console |
| `SUNSHINE_ADMOB_REWARDED_UNIT` | Google sample `ca-app-pub-3940256099942544/5224354917` | Rewarded unit. Replace with production `ca-app-pub-XXXX/YYYY` |
...
`SUNSHINE_AD_MODE` defaults to **`test`**. Android builds load a real rewarded ad through the vendored [Poing Studios AdMob plugin](https://github.com/poingstudios/godot-admob-plugin) **v4.3.1** (Godot 4.3). Editor/desktop still uses the mock overlay.

Production ids from the AdMob console (needed from Ronald/CoS):

1. `sunshine/admob_app_id` — `ca-app-pub-XXXX~YYYY` (also used in the Android manifest)
2. `sunshine/admob_rewarded_unit` — `ca-app-pub-XXXX/YYYY`
3. Set `ad_mode=live` (or `SUNSHINE_AD_MODE=live`)

Until those arrive, the APK/AAB uses Google’s official test application id `ca-app-pub-3940256099942544~3347511713` and rewarded unit `ca-app-pub-3940256099942544/5224354917`.
...
The cute-PUBG Explore multiplayer brief is **not** in 0.1.47 — that work stays a follow-up.
