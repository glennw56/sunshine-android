...
3. **DONATE** — sits **above TIP VIA AD**. Opens a blush/wine donation sheet with a progress bar toward the Square page goal (**$10,000**, `donationGoalProgress` when the hosted page is readable). Optional **Name** (leave blank to stay anonymous; last name is stored on the phone). Primary **Donate with Square** opens the existing payment link `https://square.link/u/9tUzPJZQ` in the system browser. If a name is filled we append `?name=` as a best-effort query; Square static links ignore extra keys and checkout still loads. Copy is short boilerplate Ronald can edit.
...
3. Press **F5**, or from a terminal: `godot --path .`
   You should land on the phone login (or the storefront-photo main menu if you skipped / already signed in). ORDER is the wine primary; DONATE is gold; PREVIOUS ORDERS / TIP VIA AD / EXPLORE 3D are secondary. Blush `#e8b4b8` + wine brown.
...
`feature_smoke` instantiates the menu, DONATE (existing Square link), ORDER (live Square catalog only), EXPLORE 3D (outdoor eating patio + 3 cube pastries + review cameras), and a mock staff-tip ad. `launch_smoke` presses **ORDER / DONATE / TIP VIA AD / EXPLORE 3D** for real scene changes.
