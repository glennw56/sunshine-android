...
This is a **cute third-person patio** (rounded people, real 2231 lawn menu photo) with CC0 textures (see `assets/foss/NOTICE.md`), not a photoreal remake. 0.1.56 winds up Toss cookie then releases from the hand (remotes fling from their hand with crumbs), adds rounded lawn shade trees, a Mute after patio chat, and still joins the persistent Cloud Run patio (`https://sunshine-explore-k6uuoen7wa-ue.a.run.app`). See `docs/ROOM_SERVER.md`.
...
0.1.56 winds up Toss cookie then releases from the hand; remotes skip the wind-up, hide their hand cookie, and burst crumbs on impact. Rounded lawn shade trees, a Mute after patio chat, and MSAA off for mid phones ship in this APK. The patio origin stays `https://sunshine-explore-k6uuoen7wa-ue.a.run.app` (cap 16, min 0 / max 1). See `docs/ROOM_SERVER.md` and `docs/TWO_PHONE_PATIO.md`.
