...
3. **DONATE** — lawn button **above TIP VIA AD**. Opens an in-app donation screen (blush/wine, large type) with a progress bar, optional name, and boilerplate about supporting Sunshine’s Bakery. The primary CTA opens the existing Square donation link `https://square.link/u/9tUzPJZQ` in the system browser. If Square’s public checkout page publishes a goal (it does — $10,000 for new-store improvements), the bar uses that goal and `donationGoalProgress` as raised dollars. Donor count is shown only when Square publishes it (this link does not). If the public page is unreachable, the bar is an honest placeholder toward a configurable **$500** default (`sunshine/donate_goal_cents` / `SUNSHINE_DONATE_GOAL_CENTS`). A filled name is appended as `?name=` best-effort; Square static links ignore unknown query keys and still check out. Blank name = anonymous.
...
| `SUNSHINE_DONATE_GOAL_CENTS` | `50000` | Fallback donation goal ($500) if Square’s public page has no goal |
