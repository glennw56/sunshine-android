...
4. **EXPLORE 3D** — the ChatGPT Godot bakery mesh (`assets/models/Sunshines_Bakery_Storefront_Godot4.glb`) is the walkable storefront: white + pink 2231, orange sign + logo-girl, picnic tables, mailbox, green neighbor + wood ramp. The **main menu stays the real storefront photo**. Silent on-screen left stick + look drag pad (no LOOK/MOVE coaching). Collect **3 cube pastries** for stamps. Morning **Fresh Batch** hunt stays stubbed (9–11 America/Chicago logic is still in `GameSave`). Stamp card + local weekly finder leaderboard.
