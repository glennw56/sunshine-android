...
3. **TIP VIA AD** — Android **AdMob rewarded** (Poing plugin, Godot 4.3). Credits a **FREE TIP to the STAFF jar** (not a customer perk). Release/Play default is **live** production `tip_reward` (`ca-app-pub-2788636443838183/7894363467`, app id `ca-app-pub-2788636443838183~1520526800`). `SUNSHINE_AD_MODE=test` still loads Google’s official sample rewarded unit. Editor/desktop still uses a mock overlay. Do not tap your own production ads while testing.
