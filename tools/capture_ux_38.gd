extends SceneTree
## Order/menu photos ~75% card width; Previous Orders + cart keep 96px thumbs;
## sticky cart is count-only.
##   DISPLAY=:1 godot --path . --display-driver x11 --rendering-driver opengl3 \
##     --resolution 720x1280 -s res://tools/capture_ux_38.gd
...
func _run() -> void:
	...
	if change_scene_to_file("res://scenes/order/order.tscn") != OK:
		...
		return
	await _settle(14)
	if not await _snap(disk, "order_browse_75.png"):
		quit(1)
		return
	...
	if not pick.is_empty() and order.has_method("_open_detail"):
		...
		await _settle(10)
		if not await _snap(disk, "order_item_75.png"):
			quit(1)
			return
		...
		await _settle(6)
		if not await _snap(disk, "order_sticky_count.png"):
			quit(1)
			return
	if order.has_method("_set_tab"):
		order.call("_set_tab", 1)
	await _settle(10)
	if not await _snap(disk, "cart_line_thumbs.png"):
		quit(1)
		return
	...
	if login.get("ok", false):
		...
		if prev:
			prev.pressed.emit()
		await _settle(22)
		if not await _snap(disk, "previous_orders_thumbs.png"):
			quit(1)
			return
	else:
		print("CAPTURE previous orders skipped login=", login.get("error", ""))
	print("CAPTURE 0.1.38 layout done")
	quit(0)
