...
| D2 | Cookie throw sync | Gameplay/net | D1, A3 | Same projectile both sides | Local wind-up + hand release; remotes skip wind-up, spawn from hand, crumb burst. Server `impact` relay is in this image (needs CoS `Dockerfile.explore` redeploy). Two-phone still Ronald. | review |
...
| I1 | Staging deploy + APK | Infra | A1,O1 | Installable debug APK | https://github.com/glennw56/sunshine-android/releases/download/v0.1.58-debug/sunshines-bakery-0.1.58-debug.apk | done (sideload) |
...
2. **Cookie impact broadcast** — code is in this branch (`apply_impact` + ExploreNet). Waiting on CoS `Dockerfile.explore` redeploy so live `/explore/health` shows `idle_http_seconds`.
