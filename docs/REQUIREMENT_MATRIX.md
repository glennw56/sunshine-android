...
| A2 | Forever appearance save | Identity | A1 | Survives logout; bound to Square customer | Live drinks GET/PUT `/order/api/account/avatar` (401 without session); client awaits save; patio fallback store | done (needs signed-in phone proof) |
| A3 | Visible rounded avatar + hand toss | Avatar | A1 | Third-person body; cookie leaves hand; feet on ground | `avatar_body.gd` soles at y=0, `player.gd` snap_to_ground, spring-arm camera | done (device visual still needed) |
...
| I1 | Staging deploy + APK | Infra | A1,O1 | Installable debug APK | https://github.com/glennw56/sunshine-android/releases/download/v0.1.53-debug/sunshines-bakery-0.1.53-debug.apk | done (sideload) |
