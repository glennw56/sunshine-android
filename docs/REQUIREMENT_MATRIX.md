...
| A2 | Forever appearance save | Identity | A1 | Survives logout; bound to Square customer | `profile_store.gd` vault; proposed drinks route **not live** | partial |
...
| I1 | Staging deploy + APK | Infra | A1,O1 | Installable debug APK | Built `export/sunshines-bakery-0.1.51-debug.apk` (142M, AdMob check passed). Cursor artifact store rejected 142M; no GitHub release from this agent; no Play upload | done (local export) |
...
- Apply `server/account.py` avatar routes to bakery-drinks (no extra monthly product; stays on existing Cloud Run).
- InventoryCounts on `GET /order/api/menu` so untracked/unknown can be enforced live (today drinks has no inventory object).
- Any new VM / always-on game host (likely breaks the ~$15/mo cap if left on).
