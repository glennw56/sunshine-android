...
6. **Toss cookie** on A **at B** — both phones should see B flinch/knock plus crumbs for that `proj_id`. Hits are **client-predicted** (each phone sweeps `remote_baker` / `local_baker`); the server only relays `throw` + `impact` (`hit_net_id` after CoS `Dockerfile.explore` redeploy). NPC guests still knock on grass.
